...
interface PublicOrderPaymentIslandProps {
  orderId: string;
...
export default function PublicOrderPaymentIsland({
  orderId,
  remainingBalanceRaw,
  currencyCode,
}: PublicOrderPaymentIslandProps) {
  ...
  const handlePay = async (e: Event) => {
    ...
    try {
      ...
      const res = await fetch(`/api/public/orders/${orderId}/custom-payment`, {
        method: "POST",
